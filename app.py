...
with c3:
    if st.button("🗑️ Borrar lista de registros", key="open_delete"):
...
if st.session_state["confirm_delete"]:
    st.warning("⚠️ ¿Seguro que deseas borrar **TODOS** los registros agregados? Esta acción no se puede deshacer.")
    col_yes, col_no = st.columns(2)

    with col_yes:
        if st.button("✅ Sí, borrar todo", key="confirm_yes"):
            st.session_state["registros"].clear()
            st.session_state["confirm_delete"] = False
            st.success("✅ Lista de registros borrada correctamente.")
            st.rerun()
